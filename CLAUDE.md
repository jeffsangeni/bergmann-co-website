...
- Do not assume `git push` publishes. Pushing `v2-enhanced-animations` only builds a preview; production is promoted with `vercel deploy --prod --yes` from the local clone, or (when that command is blocked) by promoting the git-built preview: `vercel promote <dpl_id> --yes`. Preview URLs are auth-protected; verify with curl against the live domain afterwards.
